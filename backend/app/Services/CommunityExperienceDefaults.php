...
class CommunityExperienceDefaults
{
    public function ensure(): void
    {
        ...
        foreach ($sources as $source) {
            CommunityExperience::firstOrCreate(['source_url' => $source['url']], [
                'content_type' => 'video', 'title' => $source['title'], 'description' => $source['description'],
                'category' => 'other', 'status' => 'published', 'verification_level' => 'source_linked',
                'moderation_note' => 'Source link supplied by the platform owner.', 'published_at' => now(),
            ]);
        }

        CommunityExperience::query()->where('source_url', 'like', '%DaKTeeDsVjY%')->delete();
    }
}
